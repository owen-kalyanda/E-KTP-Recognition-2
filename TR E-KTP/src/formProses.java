...
public class formProses extends JFrame {
	...
	private void cariPixelHorizontal(BufferedImage newImg) {
	    ...
	    //System.println("Cari pixel secara horizontal");
	    //System.println("width, height: " + w + ", " + h);
	    ...
	    for (int i = 0; i < h; i++) {
	      ...
	      //System.println("Baris ke " + i + " terdapat " + arrayPixelHorizontal[i] + " pixel biru");
	    }
	}
	...
	public static void cariPixelVertikal(BufferedImage newImg) {
	    int w = newImg.getWidth();
	    int h = newImg.getHeight();
	    //System.println("Cari pixel secara vertikal");
	    //System.println("width, height: " + w + ", " + h);
	    ...
	    for (int i = 0; i < w; i++) {
	      ...
	      //System.println("Kolom ke " + i + " terdapat " + arrayPixelVertikal[i] + " pixel Biru");
	    }
	}
	...
	private void croppingHorizontal(BufferedImage newBWImg, JLabel [] labelcrop) {
		...
		for(int i = 0; i < h; i++){
			if(arrayPixelHorizontal[i] == lblW){
				ketemuGaris++;
				//System.println("Baris ke " + i);
				//System.println("Ketemu garis " + ketemuGaris);
				if(ketemuGaris == 1){
					batasAtas = i;
				}
			}
	 ...
	 */
	public formProses() throws ClassNotFoundException, SQLException {
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		setBounds(100, 100, 1173, 464);
		...
		croppingHorizontal(textRecogEKTPUji.newBWImg, labelcrop);
		formUji fu = new formUji();
		fu.setVisible(true);
	}
}
